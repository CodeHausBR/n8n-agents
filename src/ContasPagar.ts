...
const controller_contas_pagar = class controller_contas_pagar {
    static async criar(c: t.Banco.Context) {
        try {
            ...
            const new_conta_pagar = await model_contas_pagar.criar(dados_validados, c);

            const results = {
                data: new_conta_pagar.data
            }
    ...
    static async buscar_pelo_id(c: t.Banco.Context) {
        try {
            ...
            const new_conta_pagar = await model_contas_pagar.buscar_pelo_id({ data: { id: dados_validados.data.id } }, c);

            const results = {
                data: {
                    contaPagar: new_conta_pagar.data
                }
            }
    ...
    static async atualizar_pelo_id(c: t.Banco.Context) {
        try {
            ...
            const new_conta_pagar = await model_contas_pagar.atualizar_pelo_id(dados_validados, c);
